...
import in.ashwanthkumar.notify.slack.SlackNotifierFactory
...
object Vamana extends App {
  ...
  val notifier = SlackNotifierFactory.load(vamanaConfig.notifyConfig)
  ...
  vamanaConfig.clusters.foreach(implicit cluster => {
    ...
    if(context.currentSize != newNodesCount) {
      ...
      notifier.notify(s"Updating size of ${cluster.name} from ${context.currentSize} to $newNodesCount")
